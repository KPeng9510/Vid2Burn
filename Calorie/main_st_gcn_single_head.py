...
from Calorie.dataloader_sport_skeleton import DataLoader_video_train
...
def compute_biased_loss(representation_cal, representation_word, label):
    ...
    for j in range(representation_word.size()[0]):
        ...
        for i in range(33):
            #print(i)
            cosine_similar.append(func(representation_cal[j].unsqueeze(0).cuda(), representation_word[j,i].unsqueeze(0).cuda()))
...
def compute_biased_loss_calorie(representation_cal, representation_word, label):
    ...
    for j in range(representation_word.size()[0]):
        ...
        ##print(label.size())
        #print(probs)
        loss += logloss_cuda(label[j].unsqueeze(0).cuda()+1e-8,probs.unsqueeze(0).cuda()+1e-8)
    #print(loss)
    return loss/representation_word.size()[0]
...
def logloss_cuda(y_true, y_pred, eps=1e-15):
...
def get_models(args):
    ...
    i3d = ST_GCN_18(in_channels=3, num_class=400)#generate_model(18).cuda()  #model = torch.hub.load('facebookresearch/pytorchvideo', 'slowfast_r50', pretrained=True).cuda() #generate_model(18).cuda() #torch.hub.load('facebookresearch/pytorchvideo', 'x3d_s', pretrained=True).cuda() #
...
def main(dataloaders, i3d, evaluator_action, evaluator_calorie, base_logger, args):
    ...
    optimizer = torch.optim.Adam(
        [*i3d.parameters()] + [*aggregate.parameters()] + [*evaluator_action.parameters()] + [
        ...
        for split in ['train', 'test', 'cross_test']:
            ...
            if split in ['train', 'test']:
                for data in tqdm(dataloaders[split]):
                    ...
                    probs_calorie, rep_cal= evaluator_calorie(feature)
                    ...
                    if split == 'train':
                        ...
                        loss = args.weight_calorie*loss_calorie #+ args.weight_bias* loss_bias +args.weight_output_bias*loss_calorie_bias
                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()
                        #scheduler.step()
                ...
                for data in tqdm(dataloaders[split]):
                    ...
                    #batch_size, C, frames, H, W = videos.shape
                    feature = i3d(videos)
                    probs_calorie, _ = evaluator_calorie(feature)
