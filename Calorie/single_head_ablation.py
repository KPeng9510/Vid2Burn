...
class Attention(nn.Module):
    def __init__(self, dim=400, heads = 8, dim_head = 64, dropout = 0.):
        super().__init__()
        ...
        for split in ['train', 'test', 'cross_test']:
            ...
            if split in ['train', 'test']:
                ...
                for data in pbar:
                    ...
                    #
                    emb_all_loss=0
                    probs_calorie, rep_cal = evaluator_calorie(aggregate_feature)
                    batch_size = rep_cal.size()[0]
                    for j in range(batch_size-1):
                        label = torch.argmax(data['CAL'], dim=-1).cuda()
                        if (label[j]-label[batch_size-1])<20:
                            y=1
                        else:
                            y=-1
                        emb_all_loss += emb_loss(rep_cal[batch_size-1], rep_cal[j],y)
                    ...
                    if split == 'train':
                        ...
                        #print(loss_bias)
                        loss = loss_calorie +0.5*emb_all_loss #+args.weight_output_bias*loss_calorie_bias
