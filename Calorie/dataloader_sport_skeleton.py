...
class DataLoader_video_train(Dataset):
    def __init__(self, mode, batch_size=1):
        ...
        if mode == 'train':
            ...
            self.calorie_labels = [dict[elem] for elem in self.files]
        elif mode == 'test':
    ...
    def __getitem__(self, idx):
        ...
        data_sk,_ = proc_skeleton(x_train)
        #print(data_sk.shape)
        ...
        data['filename'] = file_name.split('/')[-2] + '/' + file_name.split('/')[-1].split('.')[-2]+'.json'
...
def proc_skeleton(data):
    ...
    return skeleton_data, person_number
