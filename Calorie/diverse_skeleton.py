...
def proc_skeleton(data):
    ...
    fps = 30
    time_length =3600 / (skeleton_data.shape[0]*(1/fps))
    ...
    skeleton_movement = (0.1*np.sum(diff[:,0:4,:]/4) + 0.08*np.sum(diff[:,4:8, :]/4) + 0.11*np.sum(diff[:,8:10, :]/2) \
                        + 0.30*np.sum(diff[:,10:12, :]/2) + 0.20*np.sum(diff[:,12:14,:]/2) + 0.21*np.sum(diff[:,14:16,:]/2))*fps*fps*time_length/(4.19*1000)
    
    #skeleton_movement = np.sum(np.sqrt(skeleton_movement.sum(-1)))
    #person_number = np.stack(person_number, axis=0)

    #print(skeleton_data.shape)
    skeleton_movement=np.clip(skeleton_movement,0,1000)
    ...
    if 1:
        ...
        if 1:
            for dst in dataset:
                diff=[]
                for data in dst:
                    #print(data['filename'].split('/')[-1].split('.')[0])
                    #if data['filename'].split('/')[-1].split('.')[0]=='SchoolRulesHowTheyHelpUs_walk_f_cm_np1_fr_med_2.json':continue
                    label=np.argmax(data['CAL'], axis=-1)
                    calorie = dst.skeleton_reader(data['filename'], dst.mode)
                    diff.append(np.abs(label-calorie))
                print(np.mean(diff))
                    #po[index]+=1
