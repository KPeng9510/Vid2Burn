        ...
        for split in [ 'test', 'cross_test']:
            ...
            if split in ['train', 'test']:
                for data in tqdm(dataloaders[split]):
                    ...
                    #
                    probs_action, rep_act = evaluator_action(clip_feats.mean(1))
                    probs_calorie, rep_cal= evaluator_calorie(clip_feats.mean(1))#, emb_in) #, data['class_name'], data['calorie_prior']
                    ...
                    if split == 'train':
                        ...
                        #print(loss_bias)
                        loss = args.weight_action * loss_action + args.weight_calorie*loss_calorie+ args.weight_bias* loss_bias+args.weight_output_bias*loss_calorie_bias
                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()
                        #scheduler.step()
                ...
                for data in tqdm(dataloaders[split]):
                    ...
                    probs_calorie, _ = evaluator_calorie(clip_feats.mean(1))#,emb_in)
                    #probs_met = evaluator_met(clip_feats.mean(1))
