        ...
        for split in ['train', 'test', 'cross_test']:
            ...
            if split in ['train', 'test']:
                for data in tqdm(dataloaders[split]):
                    ...
                    probs_calorie, rep_cal= evaluator_calorie(clip_feats.mean(1)) #, data['class_name'], data['calorie_prior']
                    ...
                    if split == 'train':
                        ...
                        #print(loss_bias)
                        loss = loss_calorie +0.01*emb_all_loss/batch_size #+ args.weight_calorie*loss_calorie + args.weight_bias* loss_bias +args.weight_output_bias*loss_calorie_bias
                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()
                        #scheduler.step()
                ...
                if(split=='test'):
                    ...
                    #accuracy_score_categoties, accuracy_scxore_all = test_scikit_ap(test_label, pred_scores_action)
                    ...
                    if rho < rho_best:
                        rho_best = rho
                        epoch_best = epoch
                        log_and_print(base_logger, '-----New best found aae!-----')
                        if args.save:
