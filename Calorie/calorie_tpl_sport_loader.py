...
class DataLoader_video_train(Dataset):
    def __init__(self, mode, batch_size = 1):
        ...
        self.labels_cal = [self.calorie_annotations[elem-1] for elem in self.labels]
        self.stack_size = 90
        self.num_classes_action_labels = 27
        self.stride = 1
        self.labels_set = set(np.stack(self.labels_cal))
        self.label_to_indices = {label: np.where(np.stack(self.labels_cal) == label)[0]
                                 for label in self.labels_cal}
        if mode != 'train':
            random_state = np.random.RandomState(29)

            triplets = [[i,
                         random_state.choice(self.label_to_indices[self.labels_cal[i]]),
                         random_state.choice(self.label_to_indices[
                                                 np.random.choice(
                                                     list(self.labels_set - set([self.labels_cal[i]]))
                                                 )
                                             ])
                         ]
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            img1, label1 = self._get_video(self.files[index]), self.labels_cal[index]
            cal1=label1
            act1=self.labels[index]
            positive_index = index

            while positive_index == index:
                positive_index = np.random.choice(self.label_to_indices[label1])
            negative_label_cal = np.random.choice(list(self.labels_set - set([label1])))
            #negative_label_act = np.random.choice(list(self.labels_set - set([label1])))
            negative_index = np.random.choice(self.label_to_indices[negative_label_cal])
            img2 = self._get_video(self.files[positive_index])
            act2 = self.labels[positive_index]
            cal2 = self.calorie_annotations[act2-1]
            img3 = self._get_video(self.files[negative_index])
            act3 = self.labels[negative_index]
            cal3 = self.calorie_annotations[act3-1]
        else:
            img1 = self._get_video(self.files[self.test_triplets[index][0]])
            act1 = self.labels[self.test_triplets[index][0]]
            cal1 = self.labels_cal[self.test_triplets[index][0]]
            img2 = self._get_video(self.files[self.test_triplets[index][1]])
            act2 = self.labels[self.test_triplets[index][1]]
            cal2 = self.labels_cal[self.test_triplets[index][1]]
            img3 = self._get_video(self.files[self.test_triplets[index][2]])
            act3 = self.labels[self.test_triplets[index][2]]
            cal3 = self.labels_cal[self.test_triplets[index][2]]
        ...
        data['cal2'] = cal2
        data['cal3'] = cal3
        data['cal1_proc'] = self.proc_label(cal1, mode='CAL')
        data['cal2_proc'] = self.proc_label(cal2, mode='CAL')
        data['cal3_proc'] = self.proc_label(cal3, mode='CAL')
