...
def get_models(args):
    ...
    i3d =  generate_model(18).cuda() #torch.hub.load('facebookresearch/pytorchvideo', 'slowfast_r50', pretrained=True).cuda() #InceptionI3d().cuda()
    state_dict = torch.load(args.save_path + f'/ckpts/{args.save_model_name}.pt')
    i3d.load_state_dict(state_dict['i3d'])
    evaluator_calorie = Evaluator(output_dim=1000, model_type='USDL').cuda()
    evaluator_calorie.load_state_dict(state_dict['evaluator_calorie'])
    #evaluator_met = Evaluator(output_dim=100, model_type='USDL').cuda()
    #evaluator_action = Evaluator(output_dim=27, model_type='USDL').cuda()
    #evaluator_action.load_state_dict(state_dict['evaluator_action'])
    #if args.type == 'USDL':
    ...
    return i3d, evaluator_calorie #, evaluator_met , evaluator_action,
...
def main(dataloaders, i3d, evaluator_calorie, base_logger, args):
    # print configuration
    print('=' * 40)
        ...
        for split in ['cross_test', 'test']:
            ...
            if split in ['test', 'train']:
                for data in tqdm(dataloaders[split]):
                    ...
                    #probs_action = evaluator_action(clip_feats.mean(1))
                ...
                if(split=='test'):
                    ...
                    log_and_print(base_logger, f'{split} calorie AAE error: {aae_eval}')
                    for i in range(len(aae_cat)):
                        log_and_print(base_logger, f'{split} calorie AAE error for A{i}: {aae_cat[i]}')
                        log_and_print(base_logger, f'{split} calorie SPE error for A{i}: {rho_calorie_cat[i]}')
...
if __name__ == '__main__':
    ...
    base_logger = get_logger(save_log+f'/test{args.type}.log', args.log_info)
    i3d,  evaluator_calorie = get_models(args)
    dataloaders = get_dataloaders(args)

    main(dataloaders, i3d, evaluator_calorie, base_logger, args)
