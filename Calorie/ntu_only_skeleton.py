...
class DataLoader_video_train(Dataset):
    def __init__(self, mode, batch_size=1):
        ...
        #print(len(self.files))
        self.sklist=[]
        pa = 'ntu_staffs/ntu_calorie_all_skvideo.pkl'
        #initial_fluctuation = 50
        f=open(pa,'rb')
        self.sklist=pickle.load(f)
        ...
        if 1:
            for dst in dataset:
                ...
                for data in dst:
                    #print(data['sk'])
                    #print(data['filename'])
                    skeleton_path= '/export/md0/datasets/NTU_RGBD/skeleton/skeleton_npy/'+data['filename'].split('/')[-1].split('_')[0]+'.skeleton.npy'
                    data_y = np.load(skeleton_path, allow_pickle=True).item()
                    skeleton = data_y['skel_body0']
                    ...
                    diff = np.abs(calorie-np.argmax(data['CAL'], axis=-1))
                    resut.append(diff)
                    # identifying header
                    #header = ['gt', 'file_name']
                    #writer = csv.DictWriter(file, fieldnames=header)
                    ...
                    #writer.writeheader()
                    #if 1:
                    #    writer.writerow({'gt': np.argmax(data['CAL']).item(),'file_name': data['filena
                print(np.mean(resut))
