...
def compute_biased_loss_calorie(representation_cal, representation_word, label):
    ...
    for j in range(representation_word.size()[0]):
        ...
        #for i in range(1000):
        #print(i)
        cosine_similar = func(representation_cal[j].unsqueeze(0).repeat(1000,1).cuda()+1e-8, representation_word[j].cuda()+1e-8)
...
class lstm_aggregate(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.lstm = nn.LSTM(1039,1039,2,bidirectional=False)
...
def get_models(args):
    ...
    i3d = torch.hub.load('facebookresearch/pytorchvideo', 'slowfast_r50', pretrained=True).cuda()
    #i3d=generate_model(18).cuda() #torch.hub.load('facebookresearch/pytorchvideo', 'x3d_s', pretrained=True).cuda() #
    #i3d = InceptionI3d().cuda()
    #i3d.load_state_dict(torch.load(args.i3d_pretrained_path)['state_dict'])
    #i3d.fc = torch.nn.Identity()
    pose_model = torch.hub.load('yangsenius/TransPose:main', 'tph_a4_256x192', pretrained=True)
    pose_model.final_layer= nn.Identity()
    ...
    return i3d, pose_model, evaluator_calorie #, evaluator_met
...
def main(dataloaders, i3d, pose, evaluator_calorie, base_logger, args):
    # print configuration
    print('=' * 40)
    ...
    optimizer = torch.optim.Adam(
        [*i3d.parameters()] +[*aggregate.parameters()]+ [*pose.parameters()]   + [
    ...
    if(args.load_checkpoint == True):
        ...
        pose.load_state_dict(checkpoint['evaluator_action'])
        ...
        for split in ['train', 'test', 'cross_test']:
            ...
            if split in ['train', 'test']:
                ...
                for data in pbar:
                    ...
                    b,c,t,w,h = videos.size()
                    input = videos[:, :, ::15, :, :].permute(1,0,2,3,4).view(c,-1,w,h)
                    pose_feature = pose(input)
                    print(pose.size())
                    ...
                    """
                    #ggregate_feature = clip_feats.mean(1)
                    #print(clip_feats.size())
                    #aggregate_feature = aggregate(clip_feats)
                    #
                    #probs_calorie, rep_act = evaluator_calorie(aggregate_feature)
                    probs_calorie, rep_cal= evaluator_calorie(clip_feats.mean(1)) #, data['class_name'], data['calorie_prior'],loss_cl_rep
                    ...
                    if split == 'train':
                        ...
                        #print(loss_bias)
                        loss = loss_calorie #+ args.weight_bias* loss_bias +args.weight_output_bias*loss_calorie_bias
                ...
                for data in tqdm(dataloaders[split]):
                    ...
                    #probs_calorie, rep = evaluator_calorie(aggregate_feature)
