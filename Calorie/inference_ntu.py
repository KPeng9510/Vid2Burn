...
import torch
from ntu_calorie_sport_loader import DataLoader_video_train
...
from Calorie.ntu_calorie_sport_loader import DataLoader_video_train as dvt
...
file_name_list = ['nturgb+d_rgb/S005C003P021R001A036_rgb.avi', 'nturgb+d_rgb/S005C002P010R002A038_rgb.avi', 'nturgb+d_rgb/S003C001P019R001A035_rgb.avi', 'nturgb+d_rgb/S006C001P015R002A036_rgb.avi', 'nturgb+d_rgb/S007C002P025R001A034_rgb.avi', 'nturgb+d_rgb/S006C002P015R001A001_rgb.avi', 'nturgb+d_rgb/S008C001P015R001A027_rgb.avi','nturgb+d_rgb/S004C002P008R001A027_rgb.avi']
if __name__ == '__main__':
    ...
    for mode in [ 'cross_test']:
        ...
        for data in tqdm(dataloaders[mode]):
            #print(data.keys())
            calorie_label.append(data['real_cal'])
            ...
            #    #print(data['filename'])
                np.save(data['filename'][0].split('/')[-1]+'file.npy',map.cpu().detach().numpy())
                np.save(data['filename'][0].split('/')[-1]+'video.npy', videos.cpu().detach().numpy())
            #    #
            ...
            ##sys.exit()
        sys.exit()
        file = open('test_result_fluctuate_I3D_ntu'+mode+'.csv', 'w+', newline='')
        with file:
            # identifying header
            ...
            for i in range(len(pred_list)):
                ...
                writer.writerow({'File_name': file_list[i],
                                 'Calorie_prediction': pred_list[i],
                                 'gt':calorie_label[i]})
